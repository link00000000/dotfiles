const CACHE_DIR = ($nu.default-config-dir | path join 'cache' 'nuenv')
const NUENV_FILE = ($CACHE_DIR | path join 'allowed.txt')
...
# prints help page
export def nuenv [] {
    print "Usage: nuenv <command>

Commands
	status		Outputs the status of nuenv in the current directory
	allow		Adds `./.env.nu` to the list of allowed env files
	disallow	Removes `./.env.nu` to the list of allowed env files"
}

export def "nuenv status" [] {
    if not (".env.nu" | path exists) {
        print $"no (ansi yellow).env.nu(ansi reset) found in (ansi purple)(pwd)(ansi reset)"
    } else {
        print $"(ansi yellow).env.nu(ansi reset) found in (ansi purple)(pwd)(ansi reset)"
    }
}
...
export def "nuenv allow" [] {
    ...
    mkdir ($CACHE_DIR | path join "nuenv")
    $hash | $in + "\n" out>> $NUENV_FILE
