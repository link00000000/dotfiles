  ...
  setup_symbol_highlight_on_cursor_hold = function(client, bufnr)
    if client.server_capabilities.documentHighlightProvider then
      ...
      vim.api.nvim_create_autocmd(
        "CursorHold",
        { callback = vim.lsp.buf.document_highlight, buffer = bufnr, group = group_id }
      )
      vim.api.nvim_create_autocmd(
        "CursorHoldI",
        { callback = vim.lsp.buf.document_highlight, buffer = bufnr, group = group_id }
      ...
      vim.api.nvim_create_autocmd(
        "CursorMoved",
        { callback = vim.lsp.buf.clear_references, buffer = bufnr, group = group_id }
      )
    end
  end,
      ...
      on_attach = {
        ...
        function(client, bufnr)
          vim.keymap.set(
            ...
            { buffer = bufnr, silent = true, desc = "Show info" }
          )
        end,
      },
      handlers = {
